...
> 不一定, 在HotSpot虚拟机中, 存在JIT优化的机制, JIT优化可能会进行逃逸分析, 当经过逃逸分析发现某一个局部对象没有逃逸到线程和方法外的话, 那么这个对象就可能不会在堆上分配内存, 而是进行栈上分配.
> 
> 这里我们不多赘述逃逸分析, 这个很简单, 可以自行在该章节搜索逃逸分析的文章, 我们主要说栈上如何分配,
> 
> 首先我们要清楚一个东西
> 
> 标量替换:
> 	标量值得是一个无法再分解成更小的数据的数据, Java中的原始数据类型就是标量, 相对的, 那些还可以分解的数据叫做聚合量(Aggregate), Java中的对象就是聚合量, 因为他可以分解成其他聚合量和标量.
> 	
> 	如果在JIT阶段, 如果经过逃逸分析, 发现一个对象不会被外界访问的话, 那么经过JIT优化, 就会把这个对象拆解成若干个其中包含的若干个成员变量来替代, 这个过程就是标量替换.
> 	
> 	看一下下面的例子, Point并没有逃逸出alloc()方法, 而且, point对象是可以拆解成标量的, 那么JIT就不会在堆中创建对象, 而是会在栈上用两个标量替换这个对象
> 标量替换的好处:
> 	可以节省内存空间, 不用在堆上分配空间
> 	
> 懂了标量替换以后, 我们去看看栈上分配(其实栈上分配的本质就是标量替换)
> 
...
> 
> 我分别使用jps和jmap -histo PID来观察他们对象的数量, 本地测试, 如果不使用JIT优化, 他会很正常的将对象创建到堆中, 堆中的数量为100w, 但是如果启动逃逸分析, 从100w降到了24w, 据GC的次数来分析，也能发现，开启了逃逸分析之后，在运行期间，GC次数会明显减少。正是因为很多堆上分配被优化成了栈上分配，所以GC次数有了明显的减少。
