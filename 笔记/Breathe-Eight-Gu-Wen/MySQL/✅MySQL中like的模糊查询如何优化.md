...
- [x] MySQL中like的模糊匹配如何优化  [completion:: 2025-08-15]

> 正常来说只有后模糊匹配可以走索引, 前后都模糊和前模糊匹配都没有办法走索引, 那怎么办呢, 其实有另一种办法可以让后模糊走索引, 那就是将这个字符串反转就可以, 是不是很简单, 但是这利用了额外的空间,而且写法比较冗杂, 如果MySQL的版本高于5.6的话, 其实可以使用虚列, 这样就可以自动走索引了, 如果前模糊和后模糊两种模式相同时走索引,只需要union一下就可以了
