...
> 在8.0之前, 这句话是对的, 在8.0之后, 我们引入了索引跳跃扫描, 这句话就不一定正确了.
> 
> 因为索引跳跃扫描的优化方式就是如果你不符合最左优先匹配, 但是你的字段区分度不高, 它会将最左边的字段以它所有的value来select, 然后将结果union, 这种时候, 我们其实就可以用到索引, 虽然表面不符合, 但是最终还是符合最左前缀匹配, 然后得到对应的结果, 这种方式只适合于区分度不高的, 如果是出生年月的话, 区分度太高了, 有大量的select和union, 肯定是不能选的, 成本太高, 如果是gender, 区分度不高, 我们就可以选这种方式来进行查询, 最终到底要不要走索引跳跃扫描, 其实还是要看CBO, 优化器.
