> 为什么要使用RR呢, 我认为和主从同步有关, 一开始binlog的存储格式是statement, 其中存储的不是表的行数据, 而是执行的SQL语句, 这时候就会有并发问题, 在一个场景下两个事务, 一个事务先删除所有的记录, 另一个事务插入一条数据, 但是插入数据的事务先提交, 删除记录的事务后提交, 导致binlog在记录statement的时候, 先插入后删除, 导致主从同步出错了, 这时候该怎么办呢?就可以依靠RR模式下的GAP和next-key-lock, 这样就可以在delete的时候加gap, 在insert插入的时候就被阻塞住了, 知道事务1提交, 这时候就不会发生这种同步问题了, 但是由于之后binlog新推出了row格式, 他是记录每一行详细数据, 所以不会发生这种问题, 但是会恢复的比较慢, 所以有了fix格式.
...
所以，现在我们知道了，为什么MySQL选择RR作为默认的数据库隔离级别了吧，其实就是为了兼容历史上的那种statement格式的binlog。
