...
> ReadView, 这是一个非常重要的概念, 他和MVCC息息相关, 可以说是MVCC的基础, 实现了快照读.
> 
> 快照读就是在RC模式下, 每次读都会生成一个快照, RR模式下一个事务第一次读时会生成一个快照. 
> 
> 他是干什么的, 其实就是为了帮我们解决可见性问题, 告诉我们本个事务可以看到哪些数据, 看不到哪些数据, 可以避免不可重复读等并发问题.
> 
> ReadView如何知道事务能不能读取呢?
> 和他里面的四个参数有关系
> 
> creator_trx_id: 创建本快照的事务ID
> 
> up_limit_id: 最老的事务ID
> 
> low_limit_id: 最新的事务ID
> 
> trx_ids: 最老和最新中间的事务ID
> 
> 这四个参数主要是后三个起作用, 如果比up_limit_id更老的话, 证明那个事务在快照生成前就提交了, 所以可以看到这个快照, 如果比low_limit_id更 新的话, 就证明我们这个事务开始了, 那个新事务才创建, 自然看不见这个快照, trx_ids的作用是如果在这个列表里面, 就证明这个快照生成时这些事务还是很活跃, 所以看不到, 如果不在其中, 有比low_limit_id小的话, 就证明可以看到.
...
![[88b09d35-2a50-4874-85da-69b92c38b6c2.png]]
...
+ 3、<font style="color:#DF2A3F;">5 &le; up_limit_id &lt; 8<trx_id<low_limit_id</font>，即大于等于5，小于8，这种情况下，会再拿事务ID和Read View中的trx_ids进行逐一比较。
