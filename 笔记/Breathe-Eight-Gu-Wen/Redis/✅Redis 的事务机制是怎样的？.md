...
> redis的事务机制是保证一组操作的原子性的, 他和mysql的事务不一样, 不支持回滚
> 
> 而且会在redis的命令累计的过程中检查命令有没有错误, 如果有错误的话, 就不会去exec, 而是会抛出这个错误, 然后直接丢弃这个事务, 如果事务执行过程中发生错误, redis会继续执行剩余命令而不是回滚整个事务, 并且如果事务执行失败了, 他是会将剩下的命令执行完毕的, 不像lua脚本不会执行剩下的命令.
