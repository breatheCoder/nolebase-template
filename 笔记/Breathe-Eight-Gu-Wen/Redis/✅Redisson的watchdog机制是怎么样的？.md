...
> 为了避免redis实现的分布式锁超时, redisson引入了看门狗机制, 它可以帮我们在redisson实例关闭之前, 不断延长锁的有效期, 防止设置永不过期造成死锁, 也防止设置有时间的锁造成锁自动释放, 业务出现并发问题.
> 
> 他主要的功能就是自动续期锁, 他是如何自动续租的呢?
> 
> 他是基于netty的时间轮启动一个后台任务, 定期向redis发送命令, 重新设置锁的过期时间, 通常是锁的租约时间的 1 / 3, 这确保了即使客户端处理时间较长, 所持有的锁也不会过期.
> 
> 续期时长: 默认10s一次, 续期时长30s
> 
> 停止续期, 当锁被释放或者客户端实例被关闭, watchdog会自动停止续租任务.
> 
> 什么时候会停止续期呢?
> 
> 那就是调用unlock的时候, 然后根据threadId在redis中将那个hash结构删掉, 值得注意的是这里有一个很好的处理, 我们这个是给redis发送请求删掉, 如果调用失败没有删掉怎么办, 是不是就会unlock失败, 下次继续续期呢? 其实不是的, 在这里维护了一个本地的map结构, 他在续期和解锁的时候都会对这个map进行操作, 解锁前会先把map中的元素移除, 续期的时候不仅要注意threadId, 还要注意map中存不存在, 因为移除本地map中的操作失败的概率很低, 所以我们认为他不会失败.
