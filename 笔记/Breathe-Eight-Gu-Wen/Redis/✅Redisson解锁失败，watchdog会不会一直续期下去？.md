...
> 不会的, 我们会维护一个map结构, 即使你redis中移除锁失败了, 并且解锁失败但是他在下次续期的时候, 发现map结构中没有该锁, 就不会续期, 这样就防止了一直续期的问题
...
那么通过 [[✅Redisson的watchdog机制是怎么样的？]]我们知道，续期是需要依赖EXPIRATION_RENEWAL_MAP的，如果某个线程不在EXPIRATION_RENEWAL_MAP里面了，就不会再被续期了。
