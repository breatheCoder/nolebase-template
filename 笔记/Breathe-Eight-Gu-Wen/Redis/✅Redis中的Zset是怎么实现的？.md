...
> 这个数据结构还挺重要的, 经常用来实现排行榜的相关功能, 底层实现, 我认为skipList算是有难度的内一类了, 
> 
> 先简单说一下吧, ZSET在实现中大致分为两类
> 1. zipList + skipList
> 2. listPack + skipList
> 
> 在redis7.0开始, 不再使用zipList, 因为会导致级联问题, 开始使用listpack.
> 
> 当ZSET的元素较少的时候, Redis 会采用zipList(listpack)去存储zset的数据, ziplist(listpack)是一种紧凑的列表结构, 他通过连续存储元素来节约内存空间, 当zset的元素数量增多的时候, redis会自动将ziplist(listpack)转换为skipList, 以保持元素的有序性和支持范围查询操作.
> 
> 在这个过程中，Redis会遍历ZipList（ListPack）中的所有元素，按照元素的分数值依次将它们插入到SkipList中，这样就可以保持元素的有序性。
> 
> 接下来我们详细说说这个SKipList是如何实现的,
> 
> 其中用到了SkipList和Dict
> 
> 其中，SkipList用来实现有序集合，其中每个元素按照其分值大小在跳表中进行排序。跳表的插入、删除和查找操作的时间复杂度都是 O(log n)，可以保证较好的性能.
> 
> Dict用来实现元素和分值的映射关系, 并且效率极高, 增删改查的效率是O(1)
> 
> 然后详细介绍SkipList的结构
> 
> 他其实就是一个链表罢了, 那么为什么他的效率能达到O(logn)呢, 其实是用到了二分的思想, 他会有不同level的数据, 然后每次可以过滤出很多数据, 整个过滤流程可以把他看成一个楼梯式的结构, 所以可以提升查询效率.
