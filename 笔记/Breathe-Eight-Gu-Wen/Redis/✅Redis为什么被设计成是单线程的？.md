...
> 我们说redis单线程, 指的是他对数据的读写处理都是单线程的, 并不是说他里面没有多线程, 从6.0开始 网络IO都引入了多线程, 持久化存储模块, 集群支撑模块等都是多线程的.
> 
> 那么为什么对于数据的操作不设置成多线程呢? 这样不会更快吗?
> 
> 我们首先得直到, 计算机执行的时候什么比较耗时
> 1. 读写操作: 主要包含IO操作, 包含网络IO和磁盘IO
> 2. 计算操作: 涉及CPU.
>    
> 多线程的优势不就是通过并发的方式来提高IO的利用率和CPU的利用率.
> 
> 之所以不用多线程处理IO操作, 因为redis的操作基本都是基于内存的, 不需要频繁的从磁盘中加载, 这并不是他操作的瓶颈, 你引入多线程可能还会导致线程间频繁切换导致性能下降, CPu资源根本不是redis性能瓶颈.
