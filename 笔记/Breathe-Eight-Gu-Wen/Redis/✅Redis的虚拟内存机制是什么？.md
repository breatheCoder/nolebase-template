...
> 虚拟内存是一种节约内存的机制, 把一些不经常使用的数据存储到磁盘上, 从而避免redis占用过多的内存
> 
> 当redis使用的内存超过了指定的阈值的时候, 虚拟内存机制就自动将一些键值对转移到磁盘上, 以释放一部分内存. 当需要访问被转移磁盘上的数据时, 虚拟内存机制将自动将数据读取到内存中.
> 
> 值得注意的是2.4这个机制就已经被废弃了.
> 
> 有几点原因
> 1. 随着科技的发展, 内存空间逐渐没有我想的那么宝贵了, 减少了对虚拟内存的需求
> 2. 增大了IO负担, 如果要使用磁盘中的key, 还要再产生一次IO, 从磁盘加载到内存中, 显著降低redis的性能.
> 3. 增大了redis存储管理的复杂性.
