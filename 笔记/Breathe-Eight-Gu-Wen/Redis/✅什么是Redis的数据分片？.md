...
> redis cluster, 将redis数据集分割成多个部分, 分别存储在不同的redis节点的技术, 可以将redis数据扩展到多个物理机器上, 从而提高redis集群的性能和课扩展性.
> 
> redis数据分片的实现方式通常是按照某种规则(比如, key的hash值)分配到不同的节点, 当客户端想要访问某个key时, 会先计算应该存储在哪个节点, 然后直接连接到该节点进行操作.
> 
> redis cluster 将整个数据集划分为16384个槽(0~16383), 集群的每个节点可以负责多个hash槽, 当客户端访问数据的时候, 先根据key计算出对应的槽编号, 根据槽编号找到负责该槽的节点，向该节点发送请求.
> 
> 优点:
> 	提高了性能
> 	提高了可扩展性
...
> 
> 拓展一点知识吧
> 为什么是16384呢(2^14), 其实是和他的算法有关, 它的hash算法使用的是crc16算法, 就是在生产rdb文件也会使用的算法, crc16算法可以生产2^16 - 1次方个值, 但是为什么没有选择更大的呢? redis的创始人是这么说的: 主要是从存储和规模上说的, 存储16384占2K, 65535占8K, 此乃一胜, 规模16384可以提供1000个节点, 在真实场景下, 这绝对非常够用, 所以不需要65535, 而且65535造成节点太多, 负担有点大, 而且数据迁移不方便, 综合考虑16384是最合适的选择.
