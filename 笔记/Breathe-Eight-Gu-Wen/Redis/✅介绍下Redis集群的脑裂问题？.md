...
> 脑裂问题, 很简单的描述就是大脑裂开, 一般来说指的是一个分布式系统中有两个子集, 然后每个子集都有自己的大脑, 那么每个都认为自己是正常的, 从而导致数据不一致或者重复写入等问题.
> 
> 脑裂是如何产生的?
> 
> 我认为可能是由于主节点宕机, 然后从节点选举新的主节点的时候, 主节点恢复, 然后已经选举出新的主节点, 那么就存在两个主节点
> 
> 脑裂的不良影响?
> 
> 	重复写入
...
> 如果主从超过了这个时间, 然后从库重新选举, 如果主库恢复, 但是已经超过了这个最大延迟秒数, 所以他们不能再接收客户端请求, 也就不会发生脑裂了.
> 
> 能根本处理吗?
> 
> 其实是不行, 因为这个依赖于最大延迟秒数, 如果设置的不合理, 还是可以接收到客户端消息. 所以脑裂问题可以合理规避, 但不能完全解决.
